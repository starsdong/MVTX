...
void makeV2()
{
  ...
  TGraph *gr_sig_D_MB = (TGraph *)fin->Get("gProD0_0_80_noPid");
  TGraph *gr_sig_D_B_MB = (TGraph *)fin->Get("gNonProD0_0_80_noPid");
  ...
  const Double_t Nevt_scale = 1.0 * 0.8 ;// new significance.root file was calculated based on 240B mb, June 29, 2017, scaled to 0-80%
  ...
  for(int i=0;i<N_D;i++) {
    ...
    v2_err_D[i] = v2_err(gr_sig_D_MB->Eval(pT_D[i]), v2_D[i], EPRes) / sqrt(Nevt_scale);
    v2_D_0[i] = 0.0;
  ...
  for(int i=0;i<N_D_B;i++) {
    ...
    v2_D_B[i] = 0.;
    v2_err_D_B[i] = v2_err(gr_sig_D_B_MB->Eval(pT_D_B[i]), v2_D_B[i], EPRes) / sqrt(Nevt_scale);
  }
  ...
  TLegend *leg = new TLegend(0.5, 0.82, 0.92, 0.94);
  // leg->SetFillColor(10);
  // leg->SetFillStyle(10);
  ...
  leg->AddEntry("","Au+Au #sqrt{s_{NN}}=200 GeV, 240B MB","");
  leg->Draw();
  
  TLegend *leg = new TLegend(0.64, 0.56, 0.93, 0.8);
  // leg->SetFillColor(10);
  // leg->SetFillStyle(10);
  // leg->SetLineStyle(4000);
  // leg->SetLineColor(10);
  ...
  c1->SaveAs("fig/v2_proj_240B.eps");
  c1->SaveAs("fig/v2_proj_240B.png");
