...
void makeV1(const Bool_t sPHENIX=1)
{
  ...
  double v1slope[N_D], v1slope_err[N_D];
  
  double v1s_sum = 0;
  double v1s_err_sum = 0;
  int N_D_add = 0;
  for(int i=0;i<N_D;i++) {
    ...
    v1slope[i] = (v1_D[i] - v1_Dbar[i])/y_D[i];
    v1slope_err[i] = fabs(sqrt(v1_err_D[i]*v1_err_D[i]+v1_err_Dbar[i]*v1_err_Dbar[i])/y_D[i]);
    if(i==0 || i==1 || i==N_D-2 || i==N_D-1) {
      v1s_sum += v1slope[i];
      v1s_err_sum += v1slope_err[i]*v1slope_err[i];
      N_D_add++;
    }

    cout << y_D[i] << " " << v1_D[i] << "+/-" << v1_err_D[i] << endl;
    cout << y_D[i] << " " << v1_Dbar[i] << "+/-" << v1_err_Dbar[i] << endl;
    cout << " Slope: " << y_D[i] << " " << v1slope[i] << "+/-" << v1slope_err[i] << endl;

  }
  v1s_sum /= N_D_add;
  v1s_err_sum = sqrt(v1s_err_sum)/N_D_add;
  cout << "Total average v1slope = " << v1s_sum << " +/- " << v1s_err_sum << endl;
