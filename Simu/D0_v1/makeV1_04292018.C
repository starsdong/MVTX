...
void makeV1()
{
  ...
  // STAR data points
  TGraphErrors *gr_D_STAR = new TGraphErrors("D0v1_STAR_04132018.txt","%lg %lg %lg");
  ...
  TGraphErrors *gr_Dbar_STAR = new TGraphErrors("D0barv1_STAR_04132018.txt","%lg %lg %lg");
  ...
  for(int i=0;i<N_D;i++) {
    ...
    double sig_per_bin = sig_tot_D/sqrt((double)N_D/2.); // combine symmetric rapidity bins
  ...
  gr_Dbar_proj_2->SetMarkerStyle(24);
  ...
  TLegend *leg = new TLegend(0.12, 0.72, 0.52, 0.92);
  leg->SetFillColor(10);
  leg->SetFillStyle(10);
  ...
  TLegend *leg = new TLegend(0.72, 0.72, 0.98, 0.92);
  leg->SetFillColor(10);
  leg->SetFillStyle(10);
  leg->SetLineStyle(4000);
  leg->SetLineColor(10);
  ...
  leg->SetTextFont(42);
  leg->SetTextSize(0.045);
  leg->AddEntry(gr_D_proj,"D^{0}","pl");
  leg->AddEntry(gr_Dbar_proj,"#bar{D}^{0}","pl");
  // leg->AddEntry(gr_proj_D,"Uncert. of D^{0}","p");
  // leg->AddEntry(gr_proj_D_B,"Uncert. of D^{0} from B","p");
  leg->Draw();

  c1->SaveAs("fig/v1_proj_240B.pdf");
  c1->SaveAs("fig/v1_proj_240B.png");
